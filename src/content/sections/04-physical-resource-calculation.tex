...
\end{equation}
\Cref{eq:nisq-screen} is Stage~0: analytic triage, not a device model, fidelity estimate, or performance prediction. It does not represent coherent control error; spatial or temporal correlations, including non-Markovian noise; spectator-qubit effects and crosstalk; leakage; context-dependent gate errors; calibration drift; compiler- or schedule-dependent error; or mitigation-induced bias. Randomized-benchmarking infidelities cannot simply be substituted as per-location fault probabilities, and the coherence factor can double-count decay already present in them. Passing is necessary at this stage, never sufficient for feasibility. Failure under assumptions declared conservative for the claim may justify rejection or redesign; a favorable result must escalate before any device-performance prediction.

\paragraph{Escalation and calibration provenance.} Stage~1 freezes a timestamped, device-specific calibration snapshot and compiled-circuit model. Stage~2 interleaves circuit-matched probes with application blocks across mappings and compiler seeds. Stage~3 commits predictions before a held-out execution window or device-day. The deferred matched-\texttt{kingston} protocol in \cref{app:demo} is a specified Stage-2/3 test, not completed evidence. A snapshot is necessary for reproducibility but not sufficient for prediction: provider tables may omit coherent accumulation, crosstalk, leakage, temporal correlations, workload dependence, and within-session drift. A future vendor-neutral snapshot-and-seal adapter should place in the provenance graph: provider/backend; retrieval and calibration timestamps and a calibration/property identifier, if available; raw response, schema/API version, cryptographic digest, and missing fields; coupling map, native operations, and qubit/gate/measurement/timing properties; compiled circuit and mapping; transpiler settings and seed; runtime options; package/environment versions; and job identifiers and execution times. It must never refresh silently. Querying calibration is evidence acquisition, not verification; predictions still require circuit-matched probes and held-out outcomes.

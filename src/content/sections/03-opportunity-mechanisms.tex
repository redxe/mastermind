...
It is tempting to assign every candidate a weighted ``quantum opportunity score.'' That creates false precision. A better process is the gated sequence formalized as Algorithm~1 in \cref{sec:algorithm}: mechanism, access, output, physics, and evidence must each receive an explicit, falsifiable answer, and success at one gate cannot compensate for failure at another. If the access or output gate fails, a beautiful internal circuit is usually irrelevant. If the evidence gate fails, the project may still be good physics, compiler research, or device science, but it should not be sold as an application advantage.

...
It is tempting to assign every candidate a weighted ``quantum opportunity score.'' That creates false precision. A better process is the gated sequence formalized as Algorithm~1 in \cref{sec:algorithm}: contract, mechanism, interface, physics, and classical opponent must each receive an explicit, falsifiable answer, and success at one gate cannot compensate for failure at another. If the interface gate fails---because access or output costs consume the advantage---a beautiful internal circuit is usually irrelevant. If the evidence plan cannot be made falsifiable (\cref{sec:evidence}), the project may still be good physics, compiler research, or device science, but it should not be sold as an application advantage.

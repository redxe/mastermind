...
Equation~\eqref{eq:partition} is a design objective, not a claim that the global minimizer is analytically or efficiently available: the space of partitions is combinatorial and the cost terms are only partially known. Operationally, the audit compares a finite, predeclared set of candidate partitions, distinguishes measured costs from modeled ones, and reports sensitivity of the ordering to the modeled terms. If the available evidence cannot establish an ordering among the candidates, the partition is reported as unresolved rather than presented as $\Pi^{*}$.

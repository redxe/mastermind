...
\paragraph{Validate the audit prospectively.}

A proper inter-rater study should predeclare positive, negative, and deliberately ambiguous boundary cases, then give identical frozen evidence packets to at least three independent teams blinded to one another's decisions. Each team should seal its initial verdict, gate-by-gate reasons, and evidence citations before any consensus discussion. Report raw agreement, gate-wise confusion matrices, and Krippendorff's alpha (or a justified multi-rater alternative) with uncertainty intervals; predeclare the sample size and do not choose an acceptable coefficient after seeing results. Adjudication begins only after independent records are committed and must preserve which contract fields caused disagreement.

Agreement measures consistency, not correctness. Criterion validation follows sealed verdicts to matched end-to-end execution against the strongest classical baseline identified and actually run under the frozen selection and escalation policy, recording resources, uncertainty-qualified output, and survival or reversal of the claim. Boundary cases may remain \textsc{unresolved}; no expert answer key should force them closed.

...
The audit procedure (Algorithm 1) is useful & Author synthesis & One reproducible small-scale execution, cross-validated in a second SDK and run once on physical hardware (\cref{app:demo}); validation on an uncertain instance open (\cref{sec:audits}) \\
Noise triage predicts the direction of hardware bias & Empirical evidence (one instance, one device-day) & Single uncorrected \texttt{ibm\_kingston} run; shot-noise-only uncertainty (\cref{app:demo}) \\
Domain audits (DNA, scheduling, HEP, astronomy) & Author synthesis + provisional hypotheses & Framing analyses; quantitative rejection of phase encoding in \cref{app:demo} \\
Astronomy opportunity ranking & Provisional hypothesis & Author judgment under stated criteria (\cref{sec:astro}) \\
\bottomrule
\end{tabular}
\caption{What supports each major conclusion.}

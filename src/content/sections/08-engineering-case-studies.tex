...
\begin{enumerate}
  \item \textbf{Problem contract:} a fully instantiated $\mathcal{P}$ for one bounded instance class. \emph{Done:} the frozen instance is the 4-qubit transverse-field Ising model of \cref{app:demo} ($J=h=1$, $\epsilon=0.05$, $\delta=0.05$), fixed before any execution.
  ...
  \item \textbf{Execution and measurement:} hardware or high-fidelity-simulator runs with predeclared analysis. \emph{Done, including hardware:} seeded Aer runs reproduce the ideal energy within $0.7$ shot-noise standard errors; runs against a noise model built from real \texttt{torino} calibration data show the predicted noise bias ($\hat{E}=-4.490\pm0.026$ against $E_{\mathrm{ref}}=-4.654$, a $+0.164$ shift toward zero); and a single physical-device run on IBM's 156-qubit \texttt{ibm\_kingston} at the full Neyman budget gives $\hat{E}=-4.432\pm0.027$, the same signature at $+0.222$. Quoted uncertainties are archived shot noise only; systematic uncertainty (calibration drift, crosstalk, temporal correlations) is uncharacterized. Raw counts, job identifiers, and manifests are archived for all three.
...
The pass is complete, including a physical-device execution, and every artifact is archived. What remains open is scientific rather than procedural: the instance is exactly solvable, so the go/no-go call was never in doubt. Until the same discipline is exercised on an instance where the decision is genuinely uncertain, the framework should be read as a proposed discipline demonstrated end-to-end on a small solvable instance---a falsifiable promissory note with its first installment paid.

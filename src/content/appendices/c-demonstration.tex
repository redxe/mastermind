...
To reproduce from a clean environment, follow the paper repository's README, which documents installing the Ariadion monorepo's sibling packages together at the commit pinned at the head of this appendix and then running \texttt{scripts/demo\_tfim\_audit.py}. The emitted manifest records the resolved Ariadion commit, Python version, and full dependency lock (\texttt{pip freeze}), and every artifact file carries a SHA-256 hash.
